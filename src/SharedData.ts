...
class TestFile {
  ...
  private initialize(): void {
    if (!fs.existsSync(this.filePath)) {
      console.log(`Generating ${(FILE_SIZE / (1024 * 1024 * 1024)).toFixed(2)}GB test data file...`);
      this.generateTestData();
    } else {
      const stats = fs.statSync(this.filePath);
      if (stats.size === FILE_SIZE) {
        console.log(`Using existing test data file (${(FILE_SIZE / (1024 * 1024 * 1024)).toFixed(2)}GB)`);
